...
foreach ($xml1->xpath('//domain') as $item)	{
	...
	$html_text .= '<tr><td colspan="2"><button style="cursor:pointer;font-size:1.4rem" onclick="SwitchDisplay(11)">'.$item->url.' +/-</button></td><td><button style="cursor:pointer;font-size:1.4rem" onclick="SwitchDisplay(11)">www.'.$item->url.' +/-</button></td></tr>';
	$html_text .= '<tr id="111" style="display:table-row"><td colspan="2"><i>An apex domain is a root domain that does not contain a subdomain part.</i></td><td><i>The www subdomain has been considered unnecessary. There are some useful aspects.</i></td></tr>';
	$html_text .= '<tr id="112" style="display:table-row"><td colspan="2"><i>CNAME redirection is not allowed from the root domain.</i></td><td><i>If you host elsewhere with www. before the apex domain, then email traffic can remain safe.</i></td></tr>';
	...
	$html_text .= '<tr><td colspan="2"><button style="cursor:pointer;font-size:1.05rem" onclick="SwitchDisplay(32)">DNS MX (to direct email to a mail server) +/-</button></td><td><button style="cursor:pointer;font-size:1.05rem" onclick="SwitchDisplay(32)">DNS MX (Null MX blocks; not in cPanel) +/-</button></td></tr>';
	...
	$html_text .= '<tr><td><hr></td><td><hr></td><td><hr></td></tr>';
	$html_text .= '<tr><td colspan="3"><button style="cursor:pointer;font-size:1.05rem;font-style: italic" onclick="SwitchDisplay(34)">how to expire security.txt content +/-</button></td><td></td></tr>';
	...
	$html_text .= '<tr><td><hr></td><td><hr></td><td><hr></td></tr>';
	$html_text .= '<tr><td colspan="2"><button style="cursor:pointer;font-size:1.05rem;font-style: italic" onclick="SwitchDisplay(50)">basics for security headers +/-</button></td><td></td></tr>';
	...
	$html_text .= '<tr id="502" style="display:none"><td colspan="3"><i>Strict Transport Security over secure HTTPS is called HSTS. The server header, even of just a redirect URL, must contain an HSTS security header.</i></td></tr>';
