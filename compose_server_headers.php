...
if (strlen($DNS_CNAME))	{
	...
	if (!curl_errno($ch)) {
		...
		elseif (strlen($redirect_url) and stristr($redirect_url, 'https://'))	{
			$http_code_notice = 1;	
			$http_code_initial .= '<br />(from ' . $initial_url . ' directly to ' . $redirect_url . ')';
		}
	}
	else	{
...
if (strlen($DNS_CNAME))	{
	...
	if (!curl_errno($ch)) {
		$https_code_initial .= curl_getinfo($ch, CURLINFO_HTTP_CODE) . ' - '. 
			curl_getinfo($ch, CURLINFO_EFFECTIVE_URL) . ' redirects to ' . curl_getinfo($ch, CURLINFO_REDIRECT_URL);
	}
	else	{
...
if (strlen($DNS_CNAME_www))	{	
	...
	if (!curl_errno($ch)) {
		...
		elseif (strlen($redirect_url) and stristr($redirect_url, 'https://'))	{
			...
			$http_code_initial_www .= '<br />(from ' . $initial_url . ' directly to ' . $redirect_url . ')';
		}		
	}
	else	{
